...
static void share_vma_pages(vm_space_t *child_vm, vma_t *vma);
...
vm_space_t *create_vm_space(void)
{
    ...
    KDEBUG("create_vm_space: allocating user page directory with ASID\n");
...
static uint32_t* allocate_user_pgdir(void)
{
    ...
    uint32_t* pgdir = (uint32_t*)allocate_contiguous_pages(4);
    if (!pgdir) {
    ...
    while (parent_vma)
    {
        ...
        else
        {
            KDEBUG("fork_vm_space: SHARE VMA 0x%08X-0x%08X \n", 
                   parent_vma->start, parent_vma->end);
            share_vma_pages(child_vm, parent_vma);
            //cow_copy_vma(parent_vm, child_vm, parent_vma);
            KDEBUG(" DONE\n");
        }
...
static void share_vma_pages(vm_space_t *child_vm, vma_t *vma)
{
    ...
    for (vaddr = vma->start; vaddr < vma->end; vaddr += PAGE_SIZE)
    {
        ...
        if (!(*l1_entry & 0x1)) {
            ...
            l2_temp = map_temp_page_large(l2_phys, 12*1024);
            if (!l2_temp) {
            ...
            uint32_t temp_l1_idx = l2_temp >> 20;  // 0x80000000 >> 20 = 2048
            uint32_t temp_l2_idx = (l2_temp >> 12) & 0xFF;  // Devrait être 0
            ...
            //uint32_t kernel_l1_temp = map_temp_page((uint32_t)kernel_l1);
            ...
            if (kernel_l1_temp) {
                ...
                if (l1_entry_for_temp & 0x1) {
                    ...
                    uint32_t temp_l2_temp = map_temp_page(temp_l2_phys);
                    if (temp_l2_temp) {
                        ...
                        unmap_temp_page((void*)temp_l2_temp);
                    } else {
                ...
                unmap_temp_page((void*)kernel_l1_temp);
            } else {
...
KDEBUG("SUCCESS: Read value 0x%08X from 0x%08X\n", test_value, l2_temp);
            ...
            KDEBUG("share_vma_pages: after memset\n");
...
vma_t* find_vma(vm_space_t* vm, uint32_t addr)
{
    ...
    if (addr >= 0x40000000) {
        KDEBUG("find_vma: Address 0x%08X is in kernel space\n", addr);
        return NULL;
    }
