 ...
 *
 * File: kernel/platform/raspberrypi/irq.c
 * Layer: Kernel / BCM2836-family interrupt controller
 *
 * Responsibilities:
 ...
 * - Serve both AArch32 and AArch64 Raspberry Pi platform targets.
 */
...
#define BCM2836_IRQ_COUNTERS           64u

static volatile uint32_t bcm2836_irq_total;
static volatile uint32_t bcm2836_irq_last = 0xffffffffu;
static volatile uint32_t bcm2836_irq_counts[BCM2836_IRQ_COUNTERS];
...
{
    paddr_t physical = arch_platform_irqctrl2_phys_base();
    paddr_t section = arch_platform_irqctrl2_phys_section_base();
...
static void bcm2836_enable_local_timer_for_cpu(uint32_t cpu)
{
    if (cpu >= 4)
        return;
...
static void bcm2836_enable_local_mailbox_for_cpu(uint32_t cpu)
{
    uint32_t offset;
...
void arch_irq_controller_init(void)
{
    ...
    bcm2836_enable_local_mailbox_for_cpu(smp_processor_id());
...
{
    bcm2836_enable_local_timer_for_cpu(smp_processor_id());
    bcm2836_enable_local_mailbox_for_cpu(smp_processor_id());
}
...
{
    if (irq >= BCM2836_IRQ_COUNTERS)
        return 0;
    return bcm2836_irq_counts[irq];
}

uint32_t arch_irq_get_total_count(void)
{
    return bcm2836_irq_total;
}

uint32_t arch_irq_get_last_id(void)
{
    return bcm2836_irq_last;
}

static bool bcm2836_handle_legacy_irq(void)
{
    if (legacy_irq_pending(IRQ_UART)) {
        bcm2836_irq_last = IRQ_UART;
        bcm2836_irq_counts[IRQ_UART]++;
        uart_irq_handler();
        return true;
    }
...
static bool bcm2836_handle_mailbox0_irq(uint32_t cpu)
{
    uint32_t pending;
    ...
    bcm2836_irq_last = IRQ_SGI_TLB_SHOOTDOWN;
    if (IRQ_SGI_TLB_SHOOTDOWN < BCM2836_IRQ_COUNTERS)
        bcm2836_irq_counts[IRQ_SGI_TLB_SHOOTDOWN]++;
    smp_note_ipi(cpu);
...
void irq_c_handler(void)
{
    ...
    if (source & LOCAL_TIMER_IRQ_PHYS_BITS) {
        bcm2836_irq_last = IRQ_TIMER;
        bcm2836_irq_counts[IRQ_TIMER]++;
        timer_irq_handler();
        return;
    }

    if ((source & LOCAL_IRQ_MAILBOX0) && bcm2836_handle_mailbox0_irq(cpu))
        return;

    if ((source & LOCAL_IRQ_GPU) && bcm2836_handle_legacy_irq())
        return;

    bcm2836_irq_last = source;
}
