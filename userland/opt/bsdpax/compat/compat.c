...
static int saved_cwd_fd = -1;
static char saved_cwd_path[PATH_MAX];
...
strmode(mode_t mode, char *buf)
{
    ...
    buf[3] = (mode & S_IXUSR) ? ((mode & S_ISUID) ? 's' : 'x') :
        ((mode & S_ISUID) ? 'S' : '-');
    buf[4] = (mode & S_IRGRP) ? 'r' : '-';
    buf[5] = (mode & S_IWGRP) ? 'w' : '-';
    buf[6] = (mode & S_IXGRP) ? ((mode & S_ISGID) ? 's' : 'x') :
        ((mode & S_ISGID) ? 'S' : '-');
    buf[7] = (mode & S_IROTH) ? 'r' : '-';
    buf[8] = (mode & S_IWOTH) ? 'w' : '-';
    buf[9] = (mode & S_IXOTH) ? ((mode & S_ISVTX) ? 't' : 'x') :
        ((mode & S_ISVTX) ? 'T' : '-');
    buf[10] = ' ';
    buf[11] = '\0';
}
...
void
armos_record_fchdir(int fd)
{
    saved_cwd_fd = fd;
    if (getcwd(saved_cwd_path, sizeof(saved_cwd_path)) == NULL)
        saved_cwd_path[0] = '\0';
}
...
fchdir(int fd)
{
    if (fd != saved_cwd_fd || saved_cwd_path[0] == '\0') {
        errno = EBADF;
        return -1;
    }
    return chdir(saved_cwd_path);
}

static int
exec_with_path(const char *file, char *const argv[])
{
    const char *path;
    const char *start;
    ...
    while (1) {
        ...
        size_t dirlen = end != NULL ? (size_t)(end - start) : strlen(start);
        char full[PATH_MAX];

        if (dirlen == 0) {
            snprintf(full, sizeof(full), "%s", file);
        } else if (dirlen + 1 + strlen(file) < sizeof(full)) {
            memcpy(full, start, dirlen);
            full[dirlen] = '/';
            strcpy(full + dirlen + 1, file);
        } else {
            errno = ENAMETOOLONG;
            return -1;
        }
...
execlp(const char *file, const char *arg, ...)
{
    ...
    va_start(ap, arg);
    while (cur != NULL && argc + 1 < sizeof(argv) / sizeof(argv[0])) {
        argv[argc++] = (char *)cur;
...
char *
vis(char *dst, int c, int flags, int nextc)
{
