#!/usr/bin/env bash
# build_bsdxargs.sh - cross-build NetBSD xargs for ArmOS.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
SRC_DIR="${SRC_DIR:-$ROOT_DIR/userland/opt/bsdxargs/src}"
COMPAT_DIR="${COMPAT_DIR:-$ROOT_DIR/userland/opt/bsdxargs/compat}"
WORK_DIR="${WORK_DIR:-$ROOT_DIR/build/bsdxargs}"
BUILD_DIR="$WORK_DIR/build"
BUNDLE_ROOT="$WORK_DIR/bundle"
BUNDLE_PREFIX="$BUNDLE_ROOT/opt/bsdxargs"
...
if [ ! -f "$SRC_DIR/xargs.c" ] || [ ! -f "$SRC_DIR/strnsubst.c" ]; then
    echo "error: NetBSD xargs sources not found in $SRC_DIR" >&2
    exit 1
fi
...
objects=()
for src in xargs.c strnsubst.c; do
...
"$CC" $LDFLAGS -o "$BUNDLE_BIN/xargs" \
    $RUNTIME_OBJECTS \
    "${objects[@]}" \
    "$NEWLIB_LIBC" "$LIBGCC"

"$STRIP" --strip-all "$BUNDLE_BIN/xargs" || true

echo
echo "ArmOS BSD xargs bundle built:"
echo "  $BUNDLE_ROOT"
echo
echo "Stage with:"
echo "  rsync -a $BUNDLE_ROOT/ $ROOT_DIR/userfs/"
echo "  ln -sfn ../../opt/bsdxargs/bin/xargs $ROOT_DIR/userfs/usr/bin/xargs"
